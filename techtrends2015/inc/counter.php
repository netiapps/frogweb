...
$localhost = $databases['default']['default']['host'];
...
$dbtablehits= 'en_'.COUNT_TABLE;
$dbtableinfo= 'info';
$maxrows = 9999999999; 

function addinfo($page='homepage')
...
// ########################################################
	...
	// This page did not exsist in the counter database. A new counter must be created for this page.
		...
		//echo "INSERT INTO $dbtablehits (page, count)VALUES ('$page', '1')";
		if (!$insert) 
		{
...
if(!mysql_num_rows(mysql_query("SELECT ip_address FROM info WHERE ip_address = '$ip'"))) // check if the IP is in database
{
	...
	//echo "INSERT INTO $dbtableinfo (ip_address, user_agent, datetime) VALUES('$ip' , '$agent','$datetime' ) ";
	if (!$adddata) 
	{
